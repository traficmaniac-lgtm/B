...
class LiteAllStrategyTerminalWindow(QMainWindow):
    def __init__(
        self,
        symbol: str,
        config: Config,
        ...
        self._logger = get_logger("gui.lite_all_strategy")
        self._config = config
        ...
        self.setWindowTitle(f"Lite All Strategy Terminal v1.0 — {self._symbol}")
    ...
    def _sync_active_ids_from_open_orders(self) -> None:
        ...
        restore_prefix = f"{self._bot_order_prefix()}RESTORE_"
        tp_ids: set[str] = set()
        ...
        for order in self._open_orders_all:
            ...
            elif client_order_id.startswith(restore_prefix):
    ...
    def _place_replacement_order(self, fill: TradeFill, *, delta_qty: Decimal) -> None:
        ...
        if not allow_restore:
            if restore_reason in {"insufficient_base", "insufficient_quote"}:
                restore_reason = f"skip_{restore_reason}"
            else:
                restore_reason = "skip_insufficient"
        grid_level_key = self._format_decimal(self.q_price(restore_price, tick), tick)
        restore_client_order_id = self._limit_client_order_id(
            f"BBOT_LAS_v1_{self._symbol}_RESTORE_{restore_side}_{grid_level_key}"
        )
    ...
    def _rule_decimal(value: float | None) -> Decimal | None:
        if value is None:
            return None
        return LiteAllStrategyTerminalWindow.as_decimal(value)
    ...
    def _extract_filter_value(filters: object, filter_type: str, key: str) -> float | None:
        ...
        for entry in filters:
            ...
            if entry.get("filterType") == filter_type:
                return LiteAllStrategyTerminalWindow._coerce_float(str(entry.get(key, "")))
        return None
