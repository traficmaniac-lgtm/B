...
- лимитер stale refresh: обновления ордеров по причине `poll` запускаются только при изменениях
  или по hard‑TTL, а логирование ограничено по частоте.
...
- Ведёт активные registry‑ключи и синхронизирует их со снимком open_orders,
  предотвращая повторные RESTORE/TP на одинаковых уровнях.
...
- MarketHealth KPI объединяет спред, волатильность и edge‑метрики (edge_raw/expected_profit) для
  будущих стратегий без изменения текущей торговой логики.
